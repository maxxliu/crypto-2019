...
class Blockchain(object):
    ...
    def consensus(self, chains):
        ...
        for length in reversed(range(1, max_length+1)):
            ...
            for chain in chains.values():
                if len(chain) < length: continue
                    ...
                    if prefix_counter[prefix] >= 2: is_common = True
            if is_common: break
        ...
        result = (0,None) #save the result into a tuple
        for prefix in prefix_counter:
            if prefix_counter[prefix] >= 2:
        ...
        for node in peers:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if len(chain) > 0 and self.valid_chain(chain):
