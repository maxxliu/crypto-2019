...
def new_transaction():
    ...
    valid = blockchain.new_transaction(ast.literal_eval(values['inputs']), ast.literal_eval(values['outputs']), values['priv_key'])
    print(valid)
...
def consensus():
    # TODO does this actually handle forks?
...
if __name__ == '__main__':
    ...
    blockchain.start()
    app.run(host='0.0.0.0', port=int(args.address.split(":")[2]), debug=True, use_reloader=False)
